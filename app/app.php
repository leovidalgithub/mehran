...
?>
    ...
    <head>
        ...
        <title>Mehran Hotels</title>

        <link rel="stylesheet" type="text/css" href="./built/css/vendor.css?v=1.23">
        <link rel="stylesheet" type="text/css" href="./built/css/default.css?v=1.23">
</head>
    ...
    <body ng-controller="mainController">
        ...
        <section id="app">
            <div class="container animated">
                ...
                <div class="jumbotron">
                    <h1>MH Hotels</h1>
                    <h3 class="loggedAs" ng-cloak>Logged as: {{admin==1 ? '&#60;ADMIN&#62;' : username}}</h3>
                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        <div class="input-group-addon">
                                            <button ng-init="disablefilter = false" ng-class="{'btn-success' : !disablefilter}" class="btn btn-sm" ng-click="disablefilter = !disablefilter"
                                                title="{{disablefilter ? 'To active' : 'desactive'}} filter">
                                                <i class="glyphicon glyphicon-folder-open"></i>
                                            </button>
                                        </div>
                                        <select class="form-control input-lg" ng-model="userSelected" ng-disabled="disablefilter"
                                            id="categoriesSelect" ng-options="item as item.fullname for item in users track by item.id"
                                            ng-change="userChanged()" title="{{disablefilter ? 'Filter disabled' : ''}}">
                                        </select>
                                    </div>
                                </div>
                            </div>
        ...
        <script>
            let adminType = "<?php echo $_SESSION['type'];?>";
            let userId    = "<?php echo $_SESSION['id'];?>";
            let username  = "<?php echo $_SESSION['username'];?>";
        </script>
        
        <!-- LOADING SCRIPTS -->
        <script src="./built/js/vendor.js?v=1.23"></script>
        <script src="./built/js/built.js?v=1.23"></script>
